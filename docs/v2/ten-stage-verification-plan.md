...
- 1단계. 검증 입력 정합성 복구: 완료
- 2단계. 런타임 경로 진위 검증: 완료
- 다음 활성 단계: **3단계. widened corpus retrieval 실패 원인 해부**
...
2단계 완료 근거는 아래 문서와 리포트에 남긴다.

- [stage02-runtime-path-authenticity-report.md](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage02-runtime-path-authenticity-report.md)
- [stage02-runtime-path-authenticity-report.json](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage02-runtime-path-authenticity-report.json)

## 2026-03-31 Stage 4 Checkpoint

- 4단계 retrieval 보정 1차는 완료됐다.
- widened corpus 실패 5건을 기준으로 `source_dir/document_path` 정합성과 manifest hint rescue를 반영했다.
- 결과는 [`stage04-retrieval-correction-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage04-retrieval-correction-report.md) 와 [`stage04-policy-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage04-policy-report.json) 에 남겼다.
- Stage 4는 보정 반영 단계이고, 품질 게이트 통과를 뜻하지 않는다.
- 다음 활성 단계는 `5단계. Stage 9 retrieval / citation 회귀 검증`이다.

## 2026-03-31 Stage 5 Checkpoint

- 5단계 widened corpus Stage 9 retrieval / citation 회귀는 완료됐다.
- 전체 benchmark `13건` 기준으로 raw retrieval 과 policy-prepared retrieval 을 분리 집계했다.
- raw retrieval baseline 은 `source_dir_hit@5 = 0.0`, `supporting_doc_hit@10 = 0.0` 으로 매우 약했다.
- policy-prepared retrieval 은 `source_dir_hit@5 = 0.9231`, `supporting_doc_hit@10 = 0.9231`, `citation_correctness = 0.9231` 로 게이트를 통과했다.
- 결과는 [`stage05-stage9-regression-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage05-stage9-regression-report.md), [`stage05-policy-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage05-policy-report.json), [`stage05-regression-summary.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage05-regression-summary.json) 에 남겼다.
- 남은 리스크는 `RB-003` 1건과 raw retrieval baseline 약점이다.
- 다음 활성 단계는 `6단계. multiturn / red-team 회귀 검증`이다.

## 2026-03-31 Stage 6 Checkpoint

- 6단계 multiturn / red-team 회귀는 완료됐다.
- widened corpus retrieval 보정 이후에도 multiturn 시나리오 `2개 / 10턴` 이 모두 통과했다.
- red-team 케이스 `7건` 도 모두 통과했다.
- Stage 10 스타일 통합 판정 기준으로 `overall_decision = go` 를 확보했다.
- 결과는 [`stage06-multiturn-redteam-regression-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage06-multiturn-redteam-regression-report.md), [`stage06-multiturn-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage06-multiturn-report.json), [`stage06-red-team-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage06-red-team-report.json), [`stage06-suite-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage06-suite-report.json), [`stage06-regression-summary.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage06-regression-summary.json) 에 남겼다.
- 다음 활성 단계는 `7단계. refresh / activate / rollback 재검증`이다.

## 2026-03-31 Stage 7 Checkpoint

- 7단계 refresh / activate / rollback 재검증은 완료됐다.
- widened corpus 기준 bundle `s07r`, reindexed index `s07r-core` 로 refresh cycle 을 실제 수행했다.
- outbound / inbound validation, staging, reindex, activation, rollback evidence 를 모두 확보했다.
- 운영 포인터는 다시 `s15c-core` 로 복원됐고, `previous` 는 `s07r-core` 로 남겼다.
- source lineage 는 `openshift-docs-core-validation-20260330T090249Z` 와 `main @ b046c68e01e8032863200271caf7c95a73760364` 기준으로 유지됐다.
- Stage 7 activation 에서는 widened corpus 에서 이미 검증된 [`s15c-core-smoke-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/s15c-core-smoke-report.json) 을 동등성 확인 후 runtime authority 로 사용했다.
- Windows 환경에서 duplicate-index immediate reuse-smoke 는 비권위 artifact 로 남겼고, 최종 runtime 품질 권위는 Stage 5 / Stage 6 및 `s15c-core` smoke evidence 로 유지했다.
- 결과는 [`stage07-refresh-activate-rollback-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage07-refresh-activate-rollback-report.md) 와 [`stage07-refresh-cycle-summary.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage07-refresh-cycle-summary.json) 에 남겼다.
- 다음 활성 단계는 `8단계. live runtime / viewer / citation 경로 품질 재검증`이다.

## 2026-03-31 Stage 8 Checkpoint

- 8단계 Stage 12 live runtime 품질 재검증은 완료됐다.
- widened corpus 활성 인덱스 기준으로 live runtime smoke 를 다시 수행했고 `overall_pass = true` 를 확보했다.
- Stage 8에서는 단순 HTML 열림만 보지 않고 `section_title_present` 까지 확인하도록 viewer 검증을 강화했다.
- bridge evidence 도 pass 조건으로 올려 `company-only`, `BAAI/bge-m3`, `1024`, `upstream_chat_success_count > 0`, `fallback_chat_count = 0` 을 함께 확인했다.
- 세션 continuity 는 same conversation id 와 same session cookie value 기준으로 다시 검증했다.
- 결과는 [`stage08-live-runtime-quality-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage08-live-runtime-quality-report.md) 와 [`stage08-live-runtime-report.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage08-live-runtime-report.json) 에 남겼다.
- Stage 8은 serving-path integrity 와 citation-viewer behavior 의 pass 이며, retrieval-quality authority 는 계속 Stage 5 / Stage 6 이다.
- 다음 활성 단계는 `9단계. 저장소 정리와 v2 본체 가시화`이다.

## 2026-03-31 Stage 9 Checkpoint

- 9단계 저장소 정리와 v2 본체 가시화는 완료됐다.
- [README.md](/C:/Users/soulu/cywell/ocp-rag-chatbot/README.md) 를 제품 설명서 중심으로 다시 정리했다.
- [repository-map.md](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/repository-map.md) 를 추가해 v2 본체, 검증 자산, 생성물을 분리했다.
- [check_stage09_repository_map.py](/C:/Users/soulu/cywell/ocp-rag-chatbot/deployment/check_stage09_repository_map.py) 로 경로 정합성을 실제로 검사했다.
- 결과는 [`stage09-repository-map-check.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage09-repository-map-check.json) 과 [`stage09-repository-clarity-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage09-repository-clarity-report.md) 에 남겼다.
- 다음 활성 단계는 `10단계. 최종 통합 검증과 릴리즈 판정`이다.

## 2026-03-31 Stage 10 Checkpoint

- 10단계 최종 통합 검증과 릴리즈 판정은 완료됐다.
- Stage 5~9 결과와 현재 런타임 계약을 집계하는 [run_stage10_release_gate.py](/C:/Users/soulu/cywell/ocp-rag-chatbot/eval/run_stage10_release_gate.py) 를 추가했다.
- 최종 집계 결과는 [`stage10-final-release-gate.json`](/C:/Users/soulu/cywell/ocp-rag-chatbot/data/manifests/generated/stage10-final-release-gate.json) 에 남겼다.
- 최종 판정은 `conditional-go` 이다.
- 이 판정은 widened validation corpus 기준의 제출/시연/검증용 pass 이며, target-minor-pinned operator release 인증은 아직 포함하지 않는다.
- 최종 해석은 [`stage10-final-release-report.md`](/C:/Users/soulu/cywell/ocp-rag-chatbot/docs/v2/stage10-final-release-report.md) 에 남겼다.
- 10단계 계획은 현재 기준으로 모두 완료됐다.
