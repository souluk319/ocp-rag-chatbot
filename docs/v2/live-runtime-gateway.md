...
`OD_EMBEDDING_DIMENSIONS` exists because the active Stage 11 baseline currently uses `1024`-dimension vectors, and the embedding path is now standardized on `BAAI/bge-m3` with `1024`-dimension dense vectors computed locally through the bridge.
