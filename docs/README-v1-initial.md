...
### 주요 특징
- 프레임워크 없이 전체 RAG 파이프라인 직접 구현
- IVF(Inverted File Index) 벡터 인덱스 직접 설계 (numpy 기반 K-Means)
- Semantic + BM25 하이브리드 Reranking으로 검색 정확도 향상
...
[C0] [C1] [C2] ... [C15]    ← 16개 클러스터
 62   73   85  ...   78     ← 클러스터별 벡터 수

검색 시:
Query 벡터 → 가장 가까운 n_probe(3)개 클러스터 선택
...
└────────┬────────┘    └────────┬─────────┘
         ...
         └──────────┬───────────┘
                    ▼
            최종 Score = 0.7 × Semantic + 0.3 × BM25
                    │
                    ▼ 상위 top_k(5)개 선택
```
...
### 추가 구현
- [x] Hybrid Reranking (Semantic + BM25)
