...
def build_messages(
    *,
    ...
    system = (
        ...
        "먼저 무엇을 쓰는지 한 문장으로 말하되, 첫 문장 안에 핵심 명령이나 대상을 직접 포함한 뒤 코드 블록으로 명령이나 YAML을 제시하라. "
        "'아래 명령', '다음과 같습니다' 같은 비어 있는 표현만으로 시작하지 마라. "
        "가능하면 바로 아래에 범위, 영향, 전제, 예시 중 필요한 것만 한 줄로 덧붙여라. "
        "참조 패널은 상세 원문 확인용이다. 답변 본문은 참조 문서의 요약본처럼 쓰지 말고, 사용자가 지금 무엇을 이해하고 무엇을 해야 하는지 먼저 안내하라. "
        "근거에 명령이나 절차가 있으면 설명 요약보다 명령과 단계 위주로 재구성하라. "
        "개념 질문은 정의를 말한 뒤 실제로 어디에 왜 쓰는지까지 연결해 설명하라. "
        "트러블슈팅 질문은 확인 순서, 원인 분기, 다음 조치를 짧게 안내하라. "
    ...
    user = (
        ...
        "- 명령 질문의 첫 문장에는 핵심 명령어 또는 핵심 대상을 직접 포함할 것\n"
        "- '아래 명령', '다음 명령' 같은 표현만으로 첫 문장을 끝내지 말 것\n"
        "- 명령만 한 줄로 툭 던지지 말 것\n"
        "- 참조 패널은 원문 확인용이고, 답변 본문은 사용자가 지금 해야 할 조치와 명령을 먼저 주는 실행 가이드여야 함\n"
        "- 근거에 명령이나 절차가 있으면 문서 요약본처럼 풀어 쓰지 말고 명령과 단계 위주로 재구성할 것\n"
        "- 개념 질문에서는 정의만 말하지 말고 실제 사용 맥락과 운영상 의미를 같이 설명할 것\n"
        "- 트러블슈팅 질문에서는 단계마다 왜 필요한지와 무엇을 확인해야 하는지 1~2문장씩 덧붙일 것\n"
        "- 사용자가 참조문서를 직접 열어보더라도, 챗봇 답변은 별도의 가이드 역할을 해야 함\n"
