...
def filter_doc_to_book_hits_by_selection(
...
def doc_to_book_books_fingerprint(books_dir: Path) -> tuple[tuple[str, int], ...]:
...
def doc_to_book_row_from_section(
...
@lru_cache(maxsize=4)
def load_doc_to_book_overlay_index(
    books_dir_str: str,
    ...
    for path in sorted(books_dir.glob("*.json")):
        ...
        for section in sections:
            row = doc_to_book_row_from_section(payload, section, draft_id=draft_id)
            if row["text"]:
                rows.append(row)
    if not rows:
        return None
