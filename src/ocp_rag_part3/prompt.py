...
def build_messages(
    *,
    ...
    ops_format_rule = (
        "현재 질문이 운영 절차, 확인 방법, 수정 방법을 묻고 근거에 명령어나 절차가 있으면, "
        "'확인:', '조치:', '주의:' 형식으로 2~4개의 짧은 실무 포인트를 우선 제시하라. "
        "명령어는 필요한 줄만 코드 블록 없이 본문에 짧게 넣어도 된다. "
        if mode == "ops"
        else ""
    )
    system = (
        ...
        "답변은 항상 '답변:'으로 시작하라. "
        "명령어, 리소스 이름, 제품 고유명사를 제외한 본문은 한국어만 사용하고 중국어나 불필요한 영어 문장을 섞지 마라. "
        ...
        "단, follow-up 대명사가 세션 맥락과 해석된 질문에서 이미 해소되었고 현재 근거가 그 해석을 뒷받침하면, "
        "원문 표현이 짧거나 모호해 보여도 다시 되묻지 말고 해석된 질문 기준으로 바로 답하라. "
        f"{ops_format_rule}"
        "이전 맥락과 현재 근거가 충돌하거나 현재 근거가 비어 있으면 단정하지 말고 다시 확인하라."
    )
    session_block = f"세션 맥락:\n{session_summary}\n\n" if session_summary else ""
    resolved_query_block = ""
    cleaned_resolved_query = (resolved_query or "").strip()
    if cleaned_resolved_query and cleaned_resolved_query != query.strip():
        resolved_query_block = (
            f"해석된 질문(standalone): {cleaned_resolved_query}\n"
            "follow-up 대명사가 이미 위 질문으로 해소되었다면, 현재 근거가 같은 해석을 지지하는 한 이 질문을 기준으로 바로 답할 것.\n\n"
        )
    user = (
        f"모드: {mode}\n"
        f"답변 스타일: {style}\n"
        ...
        "- 해석된 질문(standalone)이 제공되고 현재 근거와 일치하면, 원문 대명사 때문에 clarification으로 되돌리지 말 것\n"
