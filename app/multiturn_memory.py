...
TOPIC_RULES = (
    ...
    },
    {
        ...
        "hints": ("OpenShift update", "before the update", "CNF", "etcd backup", "application pods"),
    },
    {
...
class SessionMemoryManager:
    def __init__(
        ...
        *,
        ...
        self.max_history_turns = max_history_turns or int(os.getenv("MAX_HISTORY_TURNS", "5"))
        self.session_ttl_seconds = session_ttl_seconds or int(os.getenv("SESSION_TTL_SECONDS", "3600"))
    ...
    def get_snapshot(self, session_id: str) -> SessionSnapshot:
        ...
        if snapshot is None:
            snapshot = SessionSnapshot(session_id=session_id, active_version=self.default_version)
            self._sessions[session_id] = snapshot
            return snapshot

        if snapshot.last_updated_epoch and (time.time() - snapshot.last_updated_epoch) > self.session_ttl_seconds:
            snapshot = SessionSnapshot(session_id=session_id, active_version=self.default_version)
            self._sessions[session_id] = snapshot
        return snapshot
    ...
    def process_turn(
        ...
        *,
        ...
        if classification == "follow_up" and not snapshot.active_topic and not inferred_topic:
            issues.append("follow_up_without_topic_anchor")

        if explicit_version and snapshot.active_version and explicit_version != snapshot.active_version:
            issues.append("version_switch")

        if classification == "topic_shift" and inferred_topic and snapshot.active_topic and inferred_topic != snapshot.active_topic:
            issues.append("topic_reset")

        active_topic = inferred_topic or snapshot.active_topic
        source_dir = reference_source_dir or inferred_source_dir or snapshot.source_dir
        active_version = explicit_version or snapshot.active_version or self.default_version
        hints = build_hints(
        ...
        rewritten_query = rewrite_query(
            ...
            reference_doc_path=reference_doc_path or snapshot.reference_doc_path,
        )
    ...
    def _apply_turn(self, snapshot: SessionSnapshot, turn_record: SessionTurn, reference_doc_path: str) -> None:
        ...
        snapshot.reference_doc_path = reference_doc_path or turn_record.reference_doc_path
...
def classify_turn(question_ko: str, snapshot: SessionSnapshot) -> str:
    ...
    if explicit_topic and snapshot.active_topic and explicit_topic != snapshot.active_topic:
        return "topic_shift"
