...
class TestAnsweringRoutes(unittest.TestCase):
    ...
    def test_answerer_does_not_route_image_registry_question_to_smalltalk(self) -> None:
        settings = Settings(root_dir=ROOT)
        answerer = ChatAnswerer(
            settings=settings,
            retriever=_FakeRetriever(),
        ...
        result = answerer.answer("이미지 레지스트리 저장소를 구성하려면", mode="ops")

        self.assertNotEqual("smalltalk", result.response_kind)
        self.assertNotIn("가볍게 말을 붙여도 괜찮습니다", result.answer)
    ...
    def test_answerer_shapes_etcd_backup_answer_without_proxy_overclaim(self) -> None:
        ...
        self.assertEqual("postinstallation_configuration", result.citations[0].book_slug)
        self.assertIn("etcd", [citation.book_slug for citation in result.citations])

    def test_answerer_prefers_backup_citation_when_restore_hit_precedes_backup_hit(self) -> None:
        class _RestoreThenBackupRetriever:
            def retrieve(self, query, context, top_k, candidate_k, trace_callback=None):  # noqa: ANN001
                hits = [
                    RetrievalHit(
                        chunk_id="restore-hit",
                        ...
                        viewer_path="/docs/postinstall.html#restore-state",
                        text="cluster-restore.sh /home/core/backup",
                        source="hybrid",
                        raw_score=1.0,
                        fused_score=1.0,
                    ),
                    RetrievalHit(
                        ...
                        anchor="backup-state",
                        source_url="https://example.com/postinstall",
                        viewer_path="/docs/postinstall.html#backup-state",
        ...
        class _NoCitationNarrativeLLM:
            def generate(self, messages, trace_callback=None):  # noqa: ANN001
                return "답변: 먼저 관련 절차를 확인하세요."

        settings = Settings(root_dir=ROOT)
        answerer = ChatAnswerer(
            settings=settings,
            retriever=_RestoreThenBackupRetriever(),
            llm_client=_NoCitationNarrativeLLM(),
        )

        result = answerer.answer("etcd 백업은 어떻게 해?", mode="ops")

        self.assertIn("/usr/local/bin/cluster-backup.sh /home/core/assets/backup", result.answer)
        self.assertEqual("4.12.5. etcd 데이터 백업", result.citations[0].section)
