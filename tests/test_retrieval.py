...
class RetrievalTests(unittest.TestCase):
    ...
    def test_retrieve_falls_back_to_bm25_when_vector_search_raises(self) -> None:
        ...
        settings = Settings(root_dir=ROOT)
        retriever = Part2Retriever(settings, StubBm25(), vector_retriever=ExplodingVector())

        result = retriever.retrieve("oc login usage")

        self.assertEqual("cli_tools", result.hits[0].book_slug)
        self.assertEqual(1, len(result.trace["bm25"]))
    ...
    def test_normalize_query_expands_oc_login_terms(self) -> None:
        normalized = normalize_query("oc login usage")

        for term in ("cli", "kubeconfig", "kubeadmin", "web", "browser"):
            self.assertIn(term, normalized)

    def test_normalize_query_expands_pod_pending_terms(self) -> None:
        normalized = normalize_query("Pod Pending meaning")

        for term in ("scheduling", "describe", "events", "resource", "namespace"):
            self.assertIn(term, normalized)

    def test_normalize_query_expands_crashloopbackoff_terms(self) -> None:
        normalized = normalize_query("CrashLoopBackOff troubleshooting")

        for term in ("logs", "describe", "error", "troubleshooting"):
            self.assertIn(term, normalized)

    def test_normalize_query_expands_pod_lifecycle_terms(self) -> None:
        normalized = normalize_query("Pod lifecycle concept")

        for term in ("nodes", "workloads", "initContainers", "restartPolicy", "spec", "status"):
            self.assertIn(term, normalized)
