...
class _WarningRetriever(_FakeRetriever):
    def retrieve(self, query, context, top_k, candidate_k, trace_callback=None):  # noqa: ANN001
        result = super().retrieve(query, context, top_k, candidate_k, trace_callback=trace_callback)
        result.trace["warnings"] = ["vector search failed: embedding endpoint down"]
        return result
...
class Part3AnswererTests(unittest.TestCase):
    ...
    def test_answerer_preserves_retrieval_warning_when_bm25_fallback_succeeds(self) -> None:
        settings = Settings(root_dir=ROOT)
        answerer = Part3Answerer(
            settings=settings,
