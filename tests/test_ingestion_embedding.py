...
class EmbeddingTests(unittest.TestCase):
    ...
    def test_embedding_client_uses_remote_when_load_settings_reads_dotenv_base_url(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def fake_post(url, json, headers, timeout):  # noqa: ANN001
                ...
                self.assertEqual({}, headers)
                self.assertEqual(8.0, timeout)
                self.assertEqual("dragonkue/bge-m3-ko", json["model"])
                self.assertEqual(["a", "b"], json["input"])
                return _FakeResponse(
                    200,
                    {
                        "data": [
    ...
    def test_embedding_client_fails_fast_when_remote_url_is_missing(self) -> None:
        settings = Settings(root_dir=ROOT)
        settings.embedding_base_url = ""
        settings.embedding_model = "dragonkue/bge-m3-ko"

        with self.assertRaisesRegex(RuntimeError, "Remote embedding endpoint is not configured"):
