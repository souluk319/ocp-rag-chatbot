...
class TestAppSessionFlow(unittest.TestCase):
    ...
    def test_suggest_follow_up_questions_contextualizes_generic_operator_followups(self) -> None:
        ...
        self.assertNotIn("주의사항도 함께 정리해줘", suggestions)
        self.assertNotIn("실행 예시도 같이 보여줘", suggestions)
        self.assertNotIn("상태 확인 방법도 같이 알려줘", suggestions)
        self.assertTrue(all("Operator" in item for item in suggestions))
