...
class CustomerPackScaffoldTests(unittest.TestCase):
    ...
    def test_docling_markdown_builds_structured_pdf_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            record = CustomerPackDraftStore(Path(tmpdir)).create(request)
...
"""
    ...
    def test_draft_store_persists_and_lists_customer_pack_requests(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
    ...
    def test_draft_store_preserves_historical_pack_metadata_without_silent_backfill(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            drafts_dir = root / "artifacts" / "customer_packs" / "drafts"
            ...
            legacy = CustomerPackDraftStore(root).get("dtb-legacy")

        self.assertIsNotNone(legacy)
        assert legacy is not None
        self.assertEqual("uploaded", legacy.plan.source_collection)
        self.assertEqual("custom-uploaded", legacy.plan.pack_id)
        self.assertEqual("User Custom Pack", legacy.plan.pack_label)
        self.assertEqual("unknown", legacy.plan.inferred_product)
        self.assertEqual("unknown", legacy.plan.inferred_version)
    ...
    def test_capture_service_fetches_web_html_into_capture_artifact(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            service = CustomerPackCaptureService(root)
    ...
    def test_normalize_service_builds_canonical_book_from_captured_web_html(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            capture_service = CustomerPackCaptureService(root)
            with patch("play_book_studio.ingestion.collector.requests.get", return_value=_FakeResponse()):
        ...
        self.assertIn('"viewer_path": "/playbooks/customer-packs/', payload)
