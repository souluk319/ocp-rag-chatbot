...
from _support_retrieval import RetrievalHit, SessionContext, fuse_ranked_hits
...
class TestRetrievalScoring(unittest.TestCase):
    ...
    def test_fusion_prefers_machine_config_docs_for_mco_follow_up_doc_locator(self) -> None:
        release_hit = RetrievalHit(
            chunk_id="release-notes-hit",
            book_slug="release_notes",
            chapter="릴리스 노트",
            section="4.20 릴리스 노트",
            anchor="release-notes",
            source_url="https://example.com/release-notes",
            viewer_path="/docs/release-notes.html#release-notes",
            text="4.20 릴리스 노트와 변경 사항을 설명합니다.",
            source="bm25",
            raw_score=1.0,
            fused_score=1.0,
        )
        network_hit = RetrievalHit(
            chunk_id="advanced-networking-hit",
            book_slug="advanced_networking",
            chapter="고급 네트워킹",
            section="고급 네트워킹 개요",
            anchor="advanced-networking",
            source_url="https://example.com/advanced-networking",
            viewer_path="/docs/advanced-networking.html#advanced-networking",
            text="고급 네트워킹 설정 절차를 설명합니다.",
            source="vector",
            raw_score=0.99,
            fused_score=0.99,
        )
        machine_config_hit = RetrievalHit(
            chunk_id="machine-config-hit",
            book_slug="machine_configuration",
            chapter="머신 구성",
            section="Machine Config Operator가 노드를 업데이트하는 방법",
            anchor="mco-update-flow",
            source_url="https://example.com/machine-configuration",
            viewer_path="/docs/machine-configuration.html#mco-update-flow",
            text="Machine Config Operator와 MachineConfigPool을 통해 노드 업데이트 흐름을 설명합니다.",
            source="bm25",
            raw_score=0.98,
            fused_score=0.98,
        )
        operators_hit = RetrievalHit(
            chunk_id="operators-hit",
            book_slug="operators",
            chapter="오퍼레이터",
            section="Machine Config Operator 개요",
            anchor="mco-overview",
            source_url="https://example.com/operators",
            viewer_path="/docs/operators.html#mco-overview",
            text="Machine Config Operator의 역할과 관리 지점을 설명합니다.",
            source="vector",
            raw_score=0.97,
            fused_score=0.97,
        )
        ...
        hits = fuse_ranked_hits(
            "그 설정은 어디서 바꿔? 문서 가이드 참고",
            {
                "bm25": [release_hit, network_hit, machine_config_hit, operators_hit],
                "vector": [release_hit, network_hit, machine_config_hit, operators_hit],
            },
            context=SessionContext(
                mode="ops",
                current_topic="Machine Config Operator가 노드를 업데이트하는 방법",
                open_entities=["Machine Config Operator", "machine config"],
                ocp_version="4.20",
            ),
            top_k=4,
        )

        self.assertEqual("machine_configuration", hits[0].book_slug)
        self.assertNotIn(hits[0].book_slug, {"release_notes", "advanced_networking"})
    ...
    def test_fusion_deduplicates_intake_pod_command_sections_and_keeps_concept_hits(self) -> None:
        intake_command_a = RetrievalHit(
            ...
            viewer_path="/playbooks/customer-packs/dtb-a/index.html#oc-get-pods",
        ...
        intake_command_b = RetrievalHit(
            ...
            viewer_path="/playbooks/customer-packs/dtb-b/index.html#oc-get-pods",
            text="출력 예와 함께 oc get pods 결과를 보여 줍니다. [CODE] $ oc get pods [/CODE]",
            source="overlay_bm25",
            raw_score=0.99,
            fused_score=0.99,
