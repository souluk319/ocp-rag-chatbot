...
def main() -> None:
    stage05 = load_json(
        ...
        / "manifests"
        / "generated"
        / "stage05-regression-summary.json"
    )
    stage06 = load_json(
        REPO_ROOT
        / "data"
        / "manifests"
        / "generated"
        / "stage06-regression-summary.json"
    )
    stage07 = load_json(
        REPO_ROOT
        / "data"
        / "manifests"
        / "generated"
        / "stage07-refresh-cycle-summary.json"
    )
    stage08 = load_json(
        REPO_ROOT
        / "data"
        / "manifests"
        / "generated"
        / "stage08-live-runtime-report.json"
    )
    stage09 = load_json(
        REPO_ROOT
        / "data"
        / "manifests"
        / "generated"
        / "stage09-repository-map-check.json"
    )
    ...
    previous_index = (
        (REPO_ROOT / "indexes" / "previous.txt").read_text(encoding="utf-8").strip()
    )
    ...
    gates = {
        ...
        "stage06_suite_gate": bool(stage06.get("pass"))
        and stage06.get("overall_decision") == "go",
        "stage07_refresh_gate": bool(stage07.get("pass"))
        and current_index == "s15c-core"
        and previous_index == "s07r-core",
        ...
        "runtime_contract_gate": (
            ...
            and runtime.runtime_mode() == "company-chat-plus-local-embeddings"
    ...
    if stage05["raw_summary"]["supporting_doc_hit@10"] < 0.5:
        release_risks.append(
            "raw retrieval baseline is still weak on widened corpus; current quality depends on policy-prepared retrieval"
        )
    if stage07.get("equivalent_smoke_used_for_activation"):
        release_risks.append(
            "Stage 7 activation relied on equivalent widened-corpus smoke evidence instead of a fresh duplicate-index smoke pass"
        )
    if stage08.get("bridge_health", {}).get("company_token_configured") is False:
        release_risks.append(
            "current local runtime succeeds without a configured company bearer token; future auth changes may require token provisioning"
        )
    if current_index != "s15c-core":
        release_risks.append(
            "current active index is not the validated widened corpus baseline"
        )
    if stage08.get("bridge_health", {}).get("missing_gateway_keys"):
        uncertified_items.append(
            "gateway env contract still relies on runtime injection for OD_SERVER_BASE_URL"
        )
    ...
    report = {
        ...
        "policy_summary": {
            ...
            "raw_supporting_doc_hit@10": stage05["raw_summary"][
        ...
        "refresh_summary": {
            ...
            "equivalent_smoke_used_for_activation": stage07[
                "equivalent_smoke_used_for_activation"
            ],
            "restored_runtime_authority_index_id": stage07[
                "restored_runtime_authority_index_id"
            ],
        },
        "serving_summary": {
    ...
    output_path = (
        ...
        / "manifests"
        / "generated"
        / "stage10-final-release-gate.json"
    )
