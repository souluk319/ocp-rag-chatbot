...
Updating a cluster using the web console: You can update an {product-title} cluster by using the web console. The following steps update a cluster within a minor version. You can use the same instructions for updating a cluster between minor versions.

* Performing a canary rollout update
* Pausing a MachineHealthCheck resource
* About updating {product-title} on a single-node cluster
* Updating a cluster by using the web console
* Changing the update server by using the web console

### Updating a cluster using the CLI

Updating a cluster using the CLI: You can update an {product-title} cluster within a minor version by using the OpenShift CLI (`oc`). The following steps update a cluster within a minor version. You can use the same instructions for updating a cluster between minor versions.
