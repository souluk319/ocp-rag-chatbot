...
In OpenShift Container Platform, the control plane nodes contain more than just the Kubernetes services for managing the OpenShift Container Platform cluster.
In OpenShift Container Platform, the control plane nodes are hosted in a Red Hat-owned AWS account. Red Hat fully manages the control plane infrastructure for you.
...
In OpenShift Container Platform, you can access, manage, and monitor a node through the `Node` object representing the node.
...
Container runtime:: The container runtime is responsible for running containers. OpenShift Container Platform deploys the CRI-O container runtime on each of the {op-system-first} nodes in your cluster. The Windows Machine Config Operator (WMCO) deploys the containerd runtime on its Windows nodes.
...
As an administrator, you can easily manage a node in an OpenShift Container Platform cluster
...
### Figure 1. Overview of control plane and worker node
...
### Figure 2. Overview of control plane and worker node

[FIGURE src="/playbooks/wiki-assets/full_rebuild/nodes/ROSA-HCP-overview-of-nodes-487.png" alt="Overview of control plane and worker node" kind="figure" diagram_type="image_figure"]
Overview of control plane and worker node
[/FIGURE]

_Source: `index.adoc` · asset `ROSA-HCP-overview-of-nodes-487.png`_
